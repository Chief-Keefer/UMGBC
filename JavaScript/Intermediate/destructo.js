...
console.log(`Key Locations: First = ${first}, Second = ${second}`);
...
let { upper, lower, middle = remaining.third } = doorCode;
console.log(
...
console.log(`Extracted Stones: First = ${firstStone}, Sixth = ${sixthStone}`);
...
function revealPath({ direction, distance }) {
  const { dir, dist } = { direction, distance };
  console.log(`Travel ${dist} miles ${dir}.`);
}
...
function mixPotion({ ingredient1 = "Water", ingredient2 = "Fireflower" }) {
  const { ing1, ing2 } = { ing1: ingredient1, ing2: ingredient2 };
  console.log(`Mixing Potion with: ${ing1} and ${ing2}.`);
}
...
function castSpell([ingredient1, ingredient2]) {
  const [ing1, ing2] = [ingredient1, ingredient2];
  console.log(`Casting spell with: ${ing1} and ${ing2}.`);
}
